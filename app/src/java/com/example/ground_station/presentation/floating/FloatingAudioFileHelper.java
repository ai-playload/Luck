...
public class FloatingAudioFileHelper extends BaseFloatingHelper {
    ...
    public void showFloatingAudioFile(AppCompatActivity activity, CloseCallback closeCallback) {
        ...
        EasyFloat.with(activity)
                ...
                .setDragEnable(true)
                .setTag(tag)
                .setLayout(R.layout.floating_audio_file, view -> {
                    ...
                    scaleImage.setOnScaledListener(new ScaleImage.OnScaledListener() {
                        ...
                        public void onScaled(float x, float y, MotionEvent event) {
                            ...
                            params.height = Math.max(params.height + (int) y, DisplayUtils.dpToPx(200));
...
//        RadioGroup playGroup = view.findViewById(R.id.play_group);
    ...
    private void initRemoteRecyclerView(RecyclerView remoteRecyclerView) {
        ...
        remoteAdapter.setOnItemDeleteListener( (audioModel, position) -> {
            new MaterialAlertDialogBuilder(remoteRecyclerView.getContext())
                    .setTitle("确认删除")
                    .setMessage("您确定要删除该音频吗？")
                    .setNegativeButton("取消", (dialog, which) -> {
                        // Do nothing
                    })
                    .setPositiveButton("确认", (dialog, which) -> {
                        groundStationService.sendRemoteAudioCommand(SocketConstant.PLAY_REMOTE_AUDIO_BY_INDEX, position, 5);
